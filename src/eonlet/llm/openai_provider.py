    ...
    async def stream(
        ...
        *,
        ...
        async for chunk in stream_resp:
            ...
            delta = choice.delta
            # reasoning_content is a DeepSeek thinking-mode extension; it arrives
            # before regular content and must be echoed back on subsequent turns.
    ...
    for m in messages:
        if m.role == "user":
            ...
            if m.tool_calls:
                ...
                # back whenever the assistant turn contained tool_calls.
                if m.reasoning_content:
                    entry["reasoning_content"] = m.reasoning_content
            out.append(entry)
        elif m.role == "tool":
            # OpenAI's `role=tool` message has no native is_error field, so we
            # prefix the content with an explicit marker. Without this the model
            # treats a failed tool call indistinguishably from a successful one
            # whose payload happens to look like an error string.
