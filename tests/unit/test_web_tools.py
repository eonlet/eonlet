...
def test_web_search_uses_tavily_when_key_present(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_web_fetch_handles_non_html(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_httpx(
        monkeypatch,
        get_resp=_FakeResp(text='{"k": "v"}', headers={"content-type": "application/json"}),
    )
    result = anyio.run(
        WebFetchTool().__call__, WebFetchArgs(url="http://x.test/data.json"), _ctx(tmp_path)
    )
    assert not result.is_error
