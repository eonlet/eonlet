...
def test_doctor_runs_clean_with_init(isolated_home: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_def_validate_bundled_agents(isolated_home: Path, capsys: pytest.CaptureFixture[str]) -> None:
    commands.cmd_init(force=False)
