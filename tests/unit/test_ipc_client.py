...
def test_client_demuxes_requests_and_notifications(short_tmp_path: Path) -> None:
    # ``short_tmp_path`` (see conftest) keeps the socket path well under the
    # AF_UNIX 104/108-byte cap — pytest's default ``tmp_path`` overruns it on
    # macOS CI where $TMPDIR is ``/var/folders/.../T``.
    sock = str(short_tmp_path / "s.sock")
