...
def test_progress_and_framing_not_repeated_in_kickoff() -> None:
    # The brief and trace live in the system prompt (<task_progress> /
    # <task_context>, rebuilt per turn) — the kickoff message must not carry
    # stale copies (design-review P12).
    forest = _forest(
    ...
    forest.get("t").framing = "Parent chose Postgres."  # type: ignore[union-attr]
    out = build_task_prompt(forest, "t")
    assert "got halfway" not in out
