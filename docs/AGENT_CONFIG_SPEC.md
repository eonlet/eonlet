...
MVP supports two providers; strings map directly:

- **Anthropic:** `claude-opus-4-7`, `claude-opus-4-6`, `claude-sonnet-4-6`, `claude-haiku-4-5`
- **OpenAI:** `gpt-5`, `gpt-5-mini`, `o4`, etc. (any string the OpenAI SDK accepts)
- **OpenAI-compatible endpoints** (Ollama, vLLM, LM Studio): set `model` to the model name AND set the `OPENAI_BASE_URL` and `OPENAI_API_KEY` env vars.

A future v0.2 will add a `provider` field so model strings can be unambiguous. For MVP, the prefix decides routing:
...
How the agent's memory is organized. MVP supports only procedural (Markdown) and episodic (event store) memory.

```yaml
memory:
  recent_messages_in_context: 30        # how many message events to include
  notes_files:                          # markdown files agent can read/write
    - notes.md
    - todo.md
    - last_run.md
  # semantic_store: sqlite-vec          # v0.2+
  # embedding_model: voyage-3           # v0.2+
```
...
|---|---|---|---|
| `recent_messages_in_context` | int | 30 | How many recent messages to include in context window |
| `notes_files` | list[string] | `[notes.md]` | Markdown files in `memory/` the agent reads/writes via `notes_*` tools |

### Notes files semantics

- Created at first eonlet start (touch empty file in `memory/`).
- Agent reads with `notes_read(file="notes.md")`.
- Agent writes (appends) with `notes_append(file="notes.md", content="...")`.
- **User can edit files manually** between runs — encouraged. This is how the user nudges the agent's behavior over time.

Common patterns by use case:
- **General assistant**: `notes.md` (free-form journal), `todo.md` (active tasks)
- **Scheduled digest**: `last_run.md` (state between runs), `themes.md` (recurring topics)
- **Portfolio analyst**: `watchlist.md` (tickers to monitor), `target_allocation.md` (user-edited allocation), `history.md` (trade decisions)
...
tools:
  builtin: [bash, file_read, file_write, file_edit, glob, grep,
            web_search, web_fetch, notes_read, notes_append, load_skill]
...
tools:
  builtin: [file_read, file_write, notes_read, notes_append, send_email]
  custom:
    - ./tools/x_timeline.py
...
memory:
  notes_files: [last_run.md, themes.md]
