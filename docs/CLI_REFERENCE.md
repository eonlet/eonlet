...
Refuses if eonlet status != `dead`. Run `eonlet kill <id>` first.

### `eonlet restart <id>`

Convenience: `kill` then `create` with same name and config.
...
Output: meta.json + current status + budget + recent events + memory file sizes. Useful for debugging.
...
Replay the event log to a specific point (debug-time-travel).

```bash
eonlet replay <id> [--from=<event_id>] [--to=<event_id>] [--dry-run]
```

`--dry-run` does not actually call LLMs or tools; it replays events to reconstruct state. Useful for understanding what happened.
...
eonlet create portfolio --name=main      # creates new instance
# OR resume the same one (v0.2+)
eonlet restart portfolio.main
```
