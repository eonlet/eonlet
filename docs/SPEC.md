...
- Eonlet lifecycle (`create`, `ls`, `start`, `stop`, `pause`, `resume`, `rm`)
...
`init`, `version`, `doctor`, `def ls`, `def init`, `def validate`, `def show`, `create`, `ls`, `ps`, `start`, `stop`, `pause`, `resume`, `rm`, `attach`, `send`, `logs`, `inspect`, `status`, `go`.

- `inspect <id>` — static config + resource layout (reads filesystem only).
- `status <id> [--json]` — runtime snapshot: process health, token usage, memory tier utilisation, trigger schedule, recent activity. Data sources: event store (tokens, compaction watermark, trigger_state), memory files (STM/LTM/notes/todos), IPC (triggers, live when worker is running).
...
6. **Stop is graceful** — `eonlet stop` exits within 5s; `eonlet rm` cleans up.
