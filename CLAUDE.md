...
## Project Status: v0.0.5 Landed — v0.1.0 quality bar reached
...
`src/eonlet/README.md` mirrors the planned package layout and phase-0 scope — consult it to know which module a piece of new code belongs in, and which features are explicitly **out of scope** for v0.1 (compaction, MCP, hooks, semantic memory, TUI, supervisor, A2A).

## Architecture at a Glance

Two processes per running agent:

- `eonlet` (CLI, `src/eonlet/cli/`, entry point `eonlet.cli.main:app`) — user-facing. Spawns workers, attaches/detaches via Unix sockets, manages definitions.
...
├── tools/*.py      # custom Python tools (discovered by tools/loader.py)
...
When the code lands, the project uses **uv** (not pip/poetry):
...
- Custom tools follow the protocol in `docs/TOOL_SPEC.md`; they will be discovered by `tools/loader.py` once implemented.
